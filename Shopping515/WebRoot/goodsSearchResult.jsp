...
<table width="100%" border="0" cellspacing="0" cellpadding="0" >
  ...
  <tr>
    ...
    <td><table width="100%" border="0" cellspacing="5">
      ...
      list=(List)request.getAttribute("findSearchAll");
     String keyword = new String(request.getParameter("keyword").getBytes("ISO8859-1"), "GBK");
      ...
      if(list==null){
       ...
       <%}else{ %>
        ...
        for(int i=a;i<b;i++){ %>
          <td><p> <a href=goods.do?flag=2&goodsId=<%=list.get(i).getId() %>>
          <img src="<%=list.get(i).getPic() %>" width="380" height="238" /></a></p>
            <p class="STYLE14"> <%=list.get(i).getGoodsname() %> ￥<font color=red size=4><%=list.get(i).getPrice()*list.get(i).getDiscount()/1 %> </font>价值￥<%=list.get(i).getPrice() %>
              <a href=goods.do?flag=2&goodsId=<%=list.get(i).getId() %>>去看看
              ...
              <%} %>
         <tr>
          <td colspan="4"><div align="right"><span class="STYLE3">
          <%int currentePage=(Integer)request.getAttribute("currentePage");
    int maxPage=(Integer)request.getAttribute("pageinfo");
  if(currentePage==1){%> 
     当前<%=currentePage%>页  
  <a href="goods.do?flag=3&currentePage=<%=currentePage+1 %>&keyword=<%=keyword %>">下一页&gt</a>
<a href="goods.do?flag=3&currentePage=<%=maxPage %>&keyword=<%=keyword %>">尾页</a>
   <%}else if(currentePage==maxPage){ %>  
       当前<%=currentePage%>页   
       <a href="goods.do?flag=3&currentePage=1&keyword=<%=keyword %>">首页</a>
    <a href="goods.do?flag=3&currentePage=<%=currentePage-1 %>&keyword=<%=keyword %>">&lt上一页</a>
